...
def test_login_success(client):
    ...
    assert r.status_code == 200
    assert b"TriAIlign - TaskManager" in r.data
...
def test_add_task_requires_login(client):
    r = client.post("/add", data={"title": "Should not add"}, follow_redirects=False)
    assert r.status_code == 302
    assert "/login" in r.headers["Location"]
...
def test_homepage_loads(auth_client):
    r = auth_client.get("/")
    assert r.status_code == 200
    assert b"TriAIlign - TaskManager" in r.data


def test_add_task_via_form(auth_client):
    r = auth_client.post("/add", data={"title": "UI form task", "priority": "high"}, follow_redirects=True)
    assert r.status_code == 200
